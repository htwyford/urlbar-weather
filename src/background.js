...
async function getProviderBehavior(query) {
  ...
  if (!query.searchString.includes("weather")) {
    return "inactive";
  }
...
async function getProviderResults(query) {
  ...
  let dailyForecast = [];
  for (let day = 0; day < Math.min(5, weather.data.daily.data.length); day++) {
    dailyForecast.push({
      ...
      temperatureHigh: Math.round(weather.data.daily.data[day].temperatureHigh),
...
async function getWeatherData(latitude, longitude, locationQuery) {
  ...
  const params = new URLSearchParams([
    ...
    // TODO: Consider making units a user preference.
    ["units", "auto"],
  ]);
